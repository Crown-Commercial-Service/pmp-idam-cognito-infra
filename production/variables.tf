...
variable "cognito_callback_urls_pmp_client" {
  type    = list(string)
  default = ["https://auth.print-marketplace.co.uk/auth/realms/PMP/broker/ccs/endpoint"]
}

variable "cognito_callback_urls_pmp_client_ccs" {
  type    = list(string)
  default = ["https://auth.print-marketplace.co.uk/auth/realms/PMP/broker/keycloak-oidc/endpoint"]
}
